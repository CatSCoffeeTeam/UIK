/**
 * @file UIK/Core/internal/types.hpp
 * @author CatSCoffeeTeam (contact@catscoffeeteam.com)
 * @brief This file contains global definitions used throughout the 
 *        core and applications referencing it.
 * 
 * @version 0.1.0
 * @date 2024-10-24
 * 
 * @copyright User Interface Kit is Copyright (c) CatSCoffeeTeam 2024
 */

#ifndef CatSCoffeeTeam_UIK_Core_types_h
#define CatSCoffeeTeam_UIK_Core_types_h



#include "defines.h"



namespace CatSCoffeeTeam::UIK::Core {}



#endif // CatSCoffeeTeam_UIK_Core_types_hpp